...
final class CompanyRegistrationPage extends WebPage {
	...
	static final class CompanyRegistrationForm extends WebForm {
		...
		private static final def CITY_AUTOSELCT = ".//html/body/div[5][last()]/div[@class='pac-item']/span[@class='pac-item-query']/span"
		//.//div[@class='pac-container pac-logo hdpi'][last()]/div[@class='pac-item']/span[@class='pac-item-query']/span
		//.//div[@class='pac-container pac-logo hdpi'][last()]/descendant::span[@class='pac-matched']
		...
		def static final populateFields = {browser, formData ->			
			...
			if(outcome.isSuccess()){
				CompanyRegistrationForm.mapData(browser, formData)
				println " Stored data "+CompanyRegistrationForm.mapData(browser, formData)
				for(int i = 0; i <= FIELDS.size()-1; i++){
					...
					if(FIELDS[i].equals(CITY) && formData[i]!= ""){
						...
						browser.delay(3000)
						//def location = browser.gettext(CITY_AUTOSELCT) 
						KPCommonPage.selectAutoComplete(browser, CITY_AUTOSELCT, formData[i].trim())
						//KPCommonPage.selectAutoCompleteForCity(browser, CITY_AUTOSELCT, STATE_AUTO, formData[i].trim())
						browser.delay(5000)
				...
				println "Data:::::"
				println "1 :::::::::: "+"${KPCommonPage.registrationData.values()}"
			} 
			return outcome
		}
		...
		public def static mapData(def browser, def formData){
			for(int i=0;i<FIELDS.size();i++){
				if(FIELDS[i].equals(ORGANIZATION_NAME) && formData[i]!= ""){
					KPCommonPage.registrationData << [orgName:formData[i]]
				}else
				if(FIELDS[i].equals(INDUSTRY) && formData[i]!= ""){
					KPCommonPage.registrationData << [industry:formData[i]]
				}else
				if(FIELDS[i].equals(ADDRESS_LINE1) && formData[i]!= ""){
					KPCommonPage.registrationData << [address:formData[i]]
				}else
				if(FIELDS[i].equals(CITY) && formData[i]!= ""){
					KPCommonPage.registrationData << [city:formData[i]]
				}else
				if(FIELDS[i].equals(PIN_CODE) && formData[i]!= ""){
					KPCommonPage.registrationData << [pincode:formData[i]]
				}else
				if(FIELDS[i].equals(FIRST_NAME) && formData[i]!= ""){
					KPCommonPage.registrationData << [firstName:formData[i]]
				}else
				if(FIELDS[i].equals(EMAIL_ID) && formData[i]!= ""){
					KPCommonPage.registrationData << [emailId:formData[i]]
				}else
				if(FIELDS[i].equals(MOBILE_NUM) && formData[i]!= ""){
					KPCommonPage.registrationData << [mobNum:formData[i]]
				}
			}
			println "DAta:"
			println "${KPCommonPage.registrationData.values()}"
			return KPCommonPage.registrationData
		}
	}	
}
