...
final class SearchPodPage extends WebPage {
	...
	static final class SearchPodForm extends WebForm {
		...
		private static final def ADD_TO_WISHLIST = ".//*[@id='main_page']/div[2]/div[1]/div/div[3]/div[3]/div[1]/div[3]//span"     
					...
					def allResultElement = browser.getListElements(POD_LIST)
							...
							if(allResult[0]!=null && allResultElement[0]!=null){
								if(allResult[0].trim().equalsIgnoreCase(formData[0].trim())){
									...
									return KPCommonPage.returnFailureOutcome(browser, "SearchedPodsDisplayedIssue", "Search Pod does not displayed.")
								}
							}
				}
			}else{
	...
	static final class FilterPodForm extends WebForm {
		...
		//To enter data
		def static final populateFields = { browser, formData ->
			println ("FilterPodForm.populateFields - data: " + formData)
			def outcome = WebForm.checkFormFieldsData(formData, FIELDS)
			if(outcome.isSuccess()){
				for(int i = 0; i < FIELDS.size(); i++){
					browser.delay(1000)
