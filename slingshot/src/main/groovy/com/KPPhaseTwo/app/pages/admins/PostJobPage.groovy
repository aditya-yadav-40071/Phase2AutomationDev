...
final class PostJobPage extends WebPage {
	...
	def static correctJobDisplayed = { browser, formData ->
		new PostJobForm().correctJobDisplayed browser ,formData
	}
	...
	static final class PostJobForm extends WebForm {
		...
		def static final populateFields = { browser, formData ->
			...
			if(outcome.isSuccess()){
				...
				println "THE CONTENT OF THE LIST IS---------------> "+KPCommonPage.jobDetailList
			}
			return outcome
		}
		...
		def static correctJobDisplayed = { browser, formData ->
			...
			def jobList
			def jobListElements
			browser.scrollToElement2(JOBSEARCH_TEXTBOX)
			browser.delay(1000)
			browser.populateField(JOBSEARCH_TEXTBOX,KPCommonPage.jobTitle)
			browser.delay(8000)
			jobList = browser.getLists(JOBPOSTING_LIST)
			jobListElements = browser.getListElements(JOBPOSTING_LIST)
			if(browser.isDisplayed(JOBPOSTING_LIST)){
				for(int i=0;i<jobList.size();i++){
					if(jobList[i].trim().equalsIgnoreCase(KPCommonPage.jobTitle)){
						...
						return KPCommonPage.returnFailureOutcome(browser, "JobPostIssue", "Job was not posted or was not displayed")
					}
				}
			}else{
