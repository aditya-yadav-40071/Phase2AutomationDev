...
final class PodWishlistPage extends WebPage {
	...
	def populateData = { browser, formKey, formData ->
		new PodWishListForm().populateFields(browser, formData);
	}
	
	
	def static addingPodsToWishList = { browser, formData ->
		new PodWishListForm().addingPodsToWishList browser, formData
	}
	
	def static isPodWishlistDisplayed = { browser, formData ->
		new PodWishListForm().isPodWishlistDisplayed browser, formData
	}
	
	def static getTotalViewingPodsCount ={ browser, formData ->
		new PodWishListForm().getTotalViewingPodsCount browser, formData
	}
	
	def static removeAddedPodWishlist = { browser, formData ->
		new PodWishListForm().removeAddedPodWishlist browser, formData
		
	}


	static final class PodWishListForm extends WebForm {
					...
					if(FIELDS[i].equals(FILTER_INDUSTRY)){
						if(!formData[i].equals("")){
							...
							if(formData[i].contains(",")){
								...
								KPComsmonPage.selectAutoComplete(browser, FILTER_INDUSTRY_LIST , formData[i])
							}
