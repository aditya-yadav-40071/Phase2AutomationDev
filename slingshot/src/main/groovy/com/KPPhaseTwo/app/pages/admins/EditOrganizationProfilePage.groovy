...
final class EditOrganizationProfilePage extends WebPage {
	...
	def submit = {browser, formKey, formData ->
		println ("Submit method in Edit Organization Profile Page")
		if(formKey.equals("editProfileDataSubmit")){
	...
	static final class EditOrganizationProfileForm extends WebForm {
		...
		private static def FIND_IND_CLOSE = ".//*[@id='company_profile_form_container']//span[@class='remove']/i" //"//p[contains(text(),'industryName')]/../span/i"
		...
		def static final EditOrgProfileFormErrorMessageMap = [orgName_req : "Name is required",
			...
			orgLogo_Success : "Organizaion Logo uploaded successfully"]

		//To enter data
		def static final populateFields = { browser, formData ->
			println ("EditOrgProfileForm.populateFields - data: " + formData)
			def industryList = []
		...
		def static final profileDataDisplayMatch = { browser, formData ->
			...
			println "getListOfData" +getListOfData
			formDataValues = KPCommonPage.registrationData.values()
			println "formDataValues"+formDataValues
		...
		def static final removeAllIndustry = {browser, formData ->
			...
			if(browser.isDisplayed(INDUSTRY_CLOSE)){
				...
				for(def closeElement:closeIndElements){
					browser.clickElement(closeElement)
				}
			}
		...
		def static final indErrorMessageMap = {browser, formData ->
			...
			if(industryError.equals(actualErrorMsg)){
				return new SuccessOutcome()
			}else
		...
		def static final removeIndustries = {browser, formData ->
			...
			if(formData[0].equals("")){
				...
				for(int i=0;i<industryList.size();i++){
					FIND_IND_CLOSE = FIND_IND_CLOSE.replaceAll("industryName", industryList[i])
					browser.scrollToElement2(INDUSTRY)
					browser.click FIND_IND_CLOSE
				}
				result = true
			}else{
				FIND_IND_CLOSE = FIND_IND_CLOSE.replaceAll("industryName", formData[0])
				browser.scrollToElement2(INDUSTRY)
				browser.click FIND_IND_CLOSE
				result = true
			}
		...
		def static final orgProfileImgUpload = {browser, formData ->
			...
			println browser.getCurrentDirectory()
			println dataToEnter
			...
			KPCommonPage.srcOrgLogoImage = browser.gettext(KPCommonPage.IMAGE_LOGO_ORG, "src")
			if(orgUploadText.equals(actualUploadMsg)){
				return new SuccessOutcome()
			}else {
	...
	static final class EditAboutOrganizationForm extends WebForm {
		...
		//To enter data
		def static final populateFields = { browser, formData ->
			println ("EditAboutOrganizationForm.populateFields - data: " + formData)
			def outcome = WebForm.checkFormFieldsData(formData, FIELDS)
			if(outcome.isSuccess()){
				for(int i=0;i<=FIELDS.size()-1;i++){
	...
	static final class EditLatestNewsForm extends WebForm {
		...
		//To enter data
		def static final populateFields = { browser, formData ->
			println ("EditLatestNewsForm.populateFields - data: " + formData)
			def outcome = WebForm.checkFormFieldsData(formData, FIELDS)
			if(outcome.isSuccess()){
